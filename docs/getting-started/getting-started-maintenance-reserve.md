...
When using Exchange for Auto-funding , you may want to add or update Reserve settings after the sub-merchant has been boarded. In order to do this, a maintenance case must be created.
...
In order to create maintenance case, `/maintenance` endpoint must be used with operation_type : `CREATE_MAINTENANCE`. The user will need to provide the type of maintenance type `XXXXXX`, and the `merchant_reference` for the sub-merchant. 
